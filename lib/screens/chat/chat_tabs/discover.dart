...
class _DiscoverPageState extends State<DiscoverPage> {
  ...
  Future<DateTime?> getLastActive(String uuid) async {
    var lastMessageSnapshot = await firebase.collection('chat')
            ...
            child: StreamBuilder<List<UserData>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  if (users.isEmpty) {
                    return Text("No users found", style: TextDesign().bodyTextSmall);
                  }
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return FutureBuilder<DateTime?>(
                        ...
                        builder: (context, lastActiveSnapshot) {
                          ...
                          if (lastActiveSnapshot.hasData && lastActiveSnapshot.data != null) {
                            lastActiveText = DateFormat('dd/MM/yyyy hh:mm a').format(lastActiveSnapshot.data!);
                          }
                          return Container(
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    SizedBox(width: 10),
                                    Text(user.name ?? 'Unknown User', style: TextDesign().bodyTextSmall),
                                  ],
                                ),
                                Text(lastActiveText, style: TextStyle(color: Colors.grey[600]))
                              ],
                            ),
                          );
                        },
                  ...
                  return Text("No data available", style: TextDesign().bodyTextSmall);
                }
              },
            ),
          ),
